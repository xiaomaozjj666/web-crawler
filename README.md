...
| 双击 [`启动爬虫-满血版.cmd`](启动爬虫-满血版.cmd) | 满血版（个人） | 自动设置 `WEB_CRAWLER_POWER_MODE=1`，解锁内网/元数据目标（仅本进程生效） |

两个启动器都会打开本地 Web UI（http://127.0.0.1:8765）并自动唤起浏览器；满血版开关只对这一次启动生效，不写入系统环境。
